<template>
  <div :class="[multipage === true ? 'multi-page':'single-page', 'not-menu-page', 'home-page']">
    ...
    <a-row :gutter="8" class="count-info" ref="wrapperWidth">
      <a-col :span="12" class="visit-count-wrapper">
        <a-card class="visit-count">
          <apexchart ref="count" type=bar :style="'width:' + countWidth +'px'" height=300 :options="chartOptions" :series="series" />
        </a-card>
      </a-col>
    </a-row>
  </div>
...
export default {
  ...
  methods: {
    welcome () {
      ...
      return `${this.user.username}，${time}，${welcomeArr[index]}`
    }
  },
  watch: {
    screenWidth(val){
      this.$nextTick(function () {
         this.countWidth = this.$refs.wrapperWidth.$el.clientWidth
      })
    }
  },
  mounted () {
